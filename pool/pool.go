...
func NewSinglePoolWithAddress(add string, aut ...string) *redis.Pool {
	...
	if len(aut) == 2 && len(aut[0]) != 0 && len(aut[1]) != 0 {
		opt = append(opt, redis.DialUsername(aut[0]))
		opt = append(opt, redis.DialPassword(aut[1]))
		opt = append(opt, redis.DialUseTLS(true))
	}
	...
	var p *redis.Pool
	{
		p = &redis.Pool{
			...
			Dial: func() (redis.Conn, error) {
				...
				var con redis.Conn
				{
					con, err = redis.Dial("tcp", add, opt...)
					if err != nil {
						return nil, err
					}
				}
